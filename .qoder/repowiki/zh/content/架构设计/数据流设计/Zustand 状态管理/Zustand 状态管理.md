...
## 更新摘要
**变更内容**
- 新增多 Store 集成架构说明，涵盖 useGenerationStore、useGalleryStore、useTaskStore、useSettingsStore、useUIStore 的协同工作机制
- 完善 IndexedDB 持久化策略，详细说明各 Store 与数据库层的交互模式
- 增强响应式更新机制文档，展示跨 Store 数据流和事件桥接
- 补充应用初始化流程，包括数据库启动和设置加载时序
- 优化性能考量章节，增加细粒度订阅和批量操作优化策略
...
7. [IndexedDB 持久化策略](#indexeddb-持久化策略)
8. [响应式更新机制](#响应式更新机制)
9. [依赖关系分析](#依赖关系分析)
10. [性能考量](#性能考量)
11. [故障排查指南](#故障排查指南)
...
同时详细说明 Immer 的使用方式、IndexedDB 持久化策略、多 Store 间的响应式更新机制，并给出完整的应用初始化流程和性能优化技巧。

## 项目结构
Zustand Store 位于 app/src/stores 下，每个业务域一个 store；数据库层使用 Dexie 封装 IndexedDB；任务调度由 TaskEngine 提供；常量定义在 constants/models.js；页面组件通过 hooks 订阅对应 store。
...
- **更新机制**：基于 Immer produce 进行不可变更新；异步生成流程通过 TaskEngine 执行适配器，回调中持久化到 IndexedDB，完成后回写 store
- **关键点**：支持文本到图像与图像到图像两种路径；失败时尝试将"待处理"记录标记为失败；支持扩写提示词与收藏/删除结果

### useGalleryStore  
- **职责**：图片列表、文件夹树、视图模式、搜索与过滤、多选与批量操作
- **更新机制**：从 IndexedDB 加载并按条件筛选；客户端二次过滤（如日期范围、纵横比）；批量操作后刷新列表
- **特点**：独立的图库状态域，与生成流程解耦
...
- **更新机制**：修改任一配置即持久化到 IndexedDB；启动时加载合并默认值
...
- **关键动作**
  ...
  - generate：核心生成流程，包含：
    ...
    - 提交至 TaskEngine，并在 onProgress/onTaskSubmitted 回调中持久化任务与图片
...
IsI2I --> |是| CallI2I["adapter.generateImage2Image(...)"]
IsI2I --> |否| CallT2I["adapter.generateText2Image(...)"]
CallI2I --> PersistResult["持久化图片(更新待处理或新增)"]
...
**图表来源**
- [useGenerationStore.js:135-316](file://app/src/stores/useGenerationStore.js#L135-L316)
- [database.js:154-170](file://app/src/db/database.js#L154-L170)

**章节来源**
- [useGenerationStore.js:1-386](file://app/src/stores/useGenerationStore.js#L1-L386)
- [models.js:1-110](file://app/src/constants/models.js#L1-L110)
- [database.js:43-127](file://app/src/db/database.js#L43-L127)

### useGalleryStore 分析
- **状态结构**
  - images、folders、currentFolder、viewMode、searchQuery/searchType、filters、selectedImages、isLoading
- **关键动作**
  - loadImages/loadFolders：按条件查询 IndexedDB，支持关键词搜索与多字段过滤
...
- **关键动作**
  ...
  - loadTasks：读取 IndexedDB 并计算活跃任务数
...
```mermaid
flowchart TD
Init(["应用启动"]) --> Load["loadSettings 读取 IndexedDB"]
Load --> Merge["合并默认配置"]
Merge --> Ready(["可用"])
...
### 状态同步机制
- **单向数据流**：组件 → Store → IndexedDB → Store → 组件
...
### 数据库架构
使用 Dexie.js 封装 IndexedDB，定义了以下核心表结构：
...
- [database.js:336-345](file://app/src/db/database.js#L336-L345)
...
- **外部依赖**
  ...
  - uuid：唯一 ID 生成
- **模块耦合**
  - useGenerationStore 依赖 models.js 的模型能力与默认参数，依赖 database.js 的批/图记录 API，依赖 TaskEngine 执行异步任务
  - useGalleryStore 主要依赖 database.js 的查询与批量操作
  - useTaskStore 与 TaskEngine 双向协作：事件桥 + 状态持久化
  - useSettingsStore 依赖 database.js 的 key/value 存取与 models.js 的默认配置
...
**图表来源**
- [useGenerationStore.js:14-20](file://app/src/stores/useGenerationStore.js#L14-L20)
...
- **客户端二次过滤**
  - 在 IndexedDB 查询基础上再做轻量客户端过滤，兼顾性能与交互体验
- **懒加载策略**
...
- **设置未持久化**
  - 检查 useSettingsStore.saveSettings 是否抛出异常，确认 IndexedDB 表 settings 是否存在
- **图库加载缓慢**
  - 检查 database.js 的索引与排序字段，必要时增加 limit/offset 分页
...
**章节来源**
- [useGenerationStore.js:309-315](file://app/src/stores/useGenerationStore.js#L309-L315)
...
## 结论
AI Image Studio 的 Zustand 状态管理以"领域分治 + 事件桥 + 持久化"为核心，结合 Immer 与 IndexedDB，实现了高内聚、低耦合、可恢复的前端状态体系。通过多 Store 的合理分工、细粒度订阅与合理的并发控制，系统在复杂工作流下仍保持良好性能与用户体验。建议后续继续完善错误上报、监控埋点与更丰富的调试工具。

## 附录

### 组件与状态响应式更新示例
- **工作区生成流程**
  - 组件调用 useGenerationStore.generate -> 提交 TaskEngine -> 事件回调更新 IndexedDB -> produce 更新 results/batchHistory -> UI 自动刷新
- **图库筛选流程**
  - 组件调用 useGalleryStore.filter -> 更新 filters -> 触发 loadImages -> IndexedDB 查询 -> set 更新 images -> UI 自动刷新
- **任务状态同步流程**
  - TaskEngine 事件 -> useTaskStore 监听 -> 更新 tasks 状态 -> 订阅组件自动更新

**章节来源**
- [Workbench.jsx:184-200](file://app/src/pages/Workbench.jsx#L184-L200)
- [Gallery.jsx:112-124](file://app/src/pages/Gallery.jsx#L112-L124)
