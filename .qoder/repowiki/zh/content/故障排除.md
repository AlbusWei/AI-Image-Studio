...
- [GetShortcutInfo.ps1](file://GetShortcutInfo.ps1)
- [http-backend.js](file://app/src/db/http-backend.js)
</cite>

## 更新摘要
**变更内容**   
- 新增 Windows 快捷方式配置诊断工具 GetShortcutInfo.ps1 的使用指南
- 增强缩略图生成失败和图像加载回退场景的错误处理文档
- 更新浏览器兼容性问题的诊断步骤，包含 Canvas API 和 Image 对象错误处理
- 完善存储服务的错误处理和日志记录机制说明
...
### API 代理中间件（Vite 插件 + Electron 服务器）
- 职责：读取 .env 中的密钥，按路由转发到 Qwen/EvoLink/OSS/LLM，注入 Authorization 等头，透传响应。开发环境使用 Vite 插件（api-proxy.js），生产环境使用 Electron 内嵌 HTTP 服务器（api-server.cjs）。
- 常见问题：
  - 401/403：密钥缺失或无效；Host/CORS 头不匹配。
  - 502/504：上游不可达或超时；请求体大小不一致导致服务端拒绝。
  - 跨域：开发环境由代理解决，生产部署需独立网关或反向代理。
  - 环境变量加载失败：检查 .env 文件是否在候选路径中被找到。Vite 插件使用 `import.meta.url` 推导 APP_ROOT，Electron 服务器使用多候选路径策略。
  - SSRF 403：`/api/proxy-image` 对内网地址返回 403，属于预期行为。
- 诊断要点：
  - 观察代理日志（目标 URL、请求体大小、响应状态、Content-Type）。
  - 校验环境变量前缀与命名是否与加载逻辑一致。
  - 检查启动时诊断日志中的 `envDir`/`envPath` 是否指向正确的 .env 文件。
  - API Key 在日志中已脱敏（前 4 位 + `***`），若显示完整密钥则检查脱敏逻辑。
...
- [task-engine.js:215-220](file://app/src/services/task-engine.js#L215-220)
- [task-engine.js:299-305](file://app/src/services/task-engine.js#L299-305)
...
**更新** 增强了缩略图生成失败的错误处理和图像加载回退机制

章节来源
- [storage.js:20-42](file://app/src/services/storage.js#L20-L42)
- [storage.js:138-151](file://app/src/services/storage.js#L138-L151)
- [storage.js:181-197](file://app/src/services/storage.js#L181-197)
- [storage.js:252-298](file://app/src/services/storage.js#L252-298)
...
- [qwen-adapter.js:60-105](file://app/src/services/api/qwen-adapter.js#L60-L105)
- [gpt-image-adapter.js:164-190](file://app/src/services/api/gpt-image-adapter.js#L164-L190)
- [gpt-image-adapter.js:199-241](file://app/src/services/api/gpt-image-adapter.js#L199-241)
...
- 可能原因：
  ...
  - .env 文件未被正确加载（路径推导错误、打包后缺失）。
  - Base URL 配置不合法（启动时 validateBaseUrl 报错）。
- 诊断步骤：
  - 查看代理日志的目标 URL、请求体大小、响应状态与 Content-Type。
  - 检查启动诊断日志中的 `envDir`/`envPath` 和 `loadEnv` 返回的变量。
  - 使用 ApiTest 页面逐项验证各适配器连通性。
  - 检查客户端拦截器的标准化错误对象（message/status/data）。
- 修复建议：
  - 修正环境变量命名与前缀，确保代理能读取密钥。
  - 确认 .env 文件位于 APP_ROOT（Vite 插件）或多候选路径之一（Electron 服务器）。
...
**更新** 增强了 Canvas API 和 Image 对象错误的处理机制

章节来源
- [notification.js:19-43](file://app/src/services/notification.js#L19-L43)
- [storage.js:323-347](file://app/src/services/storage.js#L323-347)
...
### Windows 快捷方式配置问题
- 现象：桌面快捷方式无法启动应用、工作目录不正确、图标显示异常。
- 可能原因：
  - 快捷方式目标路径错误。
  - 工作目录配置不正确。
  - 快捷方式参数缺失或错误。
  - 图标文件路径无效。
- 诊断步骤：
  - 使用 GetShortcutInfo.ps1 工具检查快捷方式配置详情。
  - 验证 TargetPath 指向正确的应用程序可执行文件。
  - 检查 WorkingDirectory 是否为应用根目录。
  - 确认 Arguments 参数包含必要的启动参数。
- 修复建议：
  - 重新创建快捷方式，确保所有配置项正确。
  - 使用管理员权限运行 PowerShell 脚本进行检查。
  - 验证快捷方式文件权限和完整性。

**新增** Windows 快捷方式诊断工具使用说明

章节来源
- [GetShortcutInfo.ps1:1-23](file://GetShortcutInfo.ps1#L1-L23)

### 缩略图生成失败问题
- 现象：图片缩略图无法生成、画廊显示空白、Canvas 绘制错误。
- 可能原因：
  - 图片格式不受支持或损坏。
  - Canvas API 不可用或内存不足。
  - Image 对象加载失败。
  - 浏览器安全策略限制。
- 诊断步骤：
  - 检查控制台中的 "[StorageService] Thumbnail generation failed" 错误日志。
  - 验证图片 Blob 数据完整性。
  - 测试 Canvas 2D Context 可用性。
  - 检查 Image 对象的 onload/onerror 事件处理。
- 修复建议：
  - 实现多层回退机制：优先使用存储的缩略图，其次尝试重新生成，最后使用原图。
  - 添加错误边界处理，防止单个图片失败影响整体显示。
  - 优化缩略图尺寸计算，避免超大图片导致的内存溢出。

**更新** 增强了缩略图生成的错误处理和回退机制

章节来源
- [storage.js:387-411](file://app/src/services/storage.js#L387-L411)
- [storage.js:418-432](file://app/src/services/storage.js#L418-L432)
- [http-backend.js:123-135](file://app/src/db/http-backend.js#L123-L135)
...
章节来源
- [storage.js:120-128](file://app/src/services/storage.js#L120-128)
- [storage.js:219-226](file://app/src/services/storage.js#L219-226)
- [storage.js:354-368](file://app/src/services/storage.js#L354-368)

### Windows 快捷方式诊断工具
- **GetShortcutInfo.ps1**：专门用于诊断 Windows 快捷方式配置问题的 PowerShell 工具。
- 使用方法：在 PowerShell 中运行 `.\GetShortcutInfo.ps1`，默认检查桌面上的 "AI Image Studio.lnk"。
- 输出信息：包括快捷方式路径、目标路径、工作目录、图标位置、描述和参数等详细信息。
- 适用场景：快捷方式无法启动、工作目录错误、图标显示异常等问题排查。
...
- 问题反馈：
  ...
  - 对于 Windows 快捷方式问题，使用 GetShortcutInfo.ps1 工具输出诊断信息。
- 支持渠道：
...
**更新** 增加了 Windows 快捷方式问题的诊断信息收集要求
...
**更新** 新增了 Windows 平台特定的诊断工具和缩略图生成问题的处理方法，提升了跨平台兼容性支持。